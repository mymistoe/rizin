 ...
 */
RZ_OWN RZ_API RzCoreString *rz_core_string_information(RzCore *core, const char *block, int len, RzCoreStringKind kind) {
	...
	if (kind != RZ_CORE_STRING_KIND_UNKNOWN) {
		...
		cstring->kind = get_string_kind(core->block, len);
	}
